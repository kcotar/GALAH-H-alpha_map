...
if PLOT_LOSS:
    ...
    plt.plot(x_loss, ann_fit_loss1[:, 1], label='Training (blue arm)', lw=0.75, c='C0')
    plt.plot(x_loss, ann_fit_loss1[:, 0], label='Validation (blue arm)',lw=0.75, ls='--', c='C0')
    plt.plot(x_loss, ann_fit_loss3[:, 1], label='Training (red arm)', lw=0.75, c='C3')
    plt.plot(x_loss, ann_fit_loss3[:, 0], label='Validation (red arm)', lw=0.75, ls='--', c='C3')
...
if PLOT_SPECTRA:
    ...
    for it, clabel in enumerate(['Drifting peak location', 'Drifting peak location', 'Variable intensity', 'Reduction induced']):
        ax[it].text(plot_wvl - wvl_plot_span/3 + 2, 0.45, clabel,
    ...
    for it, clabel in enumerate(['Cold star', 'Fast rotator', 'SB2 binary', r'H$\alpha$/H$\beta$ emission']):
        ax[it].text(plot_wvl - wvl_plot_span + 2, 0.35, clabel,
                    verticalalignment='center')
